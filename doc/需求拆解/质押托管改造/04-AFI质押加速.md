...
- AFI 加速配置的质押比例和加速倍率属于可变配置；用户质押成功时，命中的 `pledge_ratio` 和 `accelerate_rate` 必须写入质押记录，后续定时收益任务读取质押记录快照，不受配置表后续调整影响。
...
- `pledge_ratio`：命中质押比例快照，单位 %
- `accelerate_rate`：命中加速倍率快照
- `pledge_time`
...
- 将命中的 `pledge_ratio` 和 `accelerate_rate` 写入质押记录，作为后续收益加速快照。
...
- 首次命中 `effective_day <= 当前日期` 的质押记录时，将质押记录状态从 `未生效` 更新为 `生效中`。
